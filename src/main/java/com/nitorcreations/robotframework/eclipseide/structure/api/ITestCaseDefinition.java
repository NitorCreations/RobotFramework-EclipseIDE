...
public interface ITestCaseDefinition extends IKeywordSequence {
     ...
     * the tag set can also be manipulated dynamically when the test case
     * executes. These are not included in the returned set, only the "static"
    ...
    /**
     * Since Robot Framework 2.5.6, the {@link IKeywordCall#getKeyword()} may
     * exceptionally return one of the special values "" or "NONE" when the user
     * wants to override the file-default test setup with a no-op. (It's unknown
     * why the "No Operation" keyword wasn't reused).
     */
    IKeywordCall getTestSetup();

    /**
     * Same special circumstances as documented in {@link #getTestSetup()}
     * apply.
     */
    IKeywordCall getTestTeardown();

    /**
     * Since Robot Framework 2.5.6, the special template "NONE" indicates no
