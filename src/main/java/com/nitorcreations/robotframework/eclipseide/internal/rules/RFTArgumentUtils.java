...
public final class RFTArgumentUtils {
     ...
     * given. If only whitespace is found, "no keyword" is returned. Also if the
     * given start position does not start with a whitespace separator,
     * "no keyword" is returned.
     * 
     * @param line
     *            the line to scan
     * @param start
     *            the position to start scanning on. Must be either the start of
     *            line or the first whitespace detected after the previous
     *            argument.
     * @return the position at which the next keyword starts, or -1 if no more
     *         keywords are found.
     */
    ...
    public static int calculateArgumentLength(String line, int start) {
        ...
        outer: while (true) {
            ...
            if (end + 1 == line.length()) {
                // break, retaining end at the whitespace position, excluding it
                // from the keyword (Robot
