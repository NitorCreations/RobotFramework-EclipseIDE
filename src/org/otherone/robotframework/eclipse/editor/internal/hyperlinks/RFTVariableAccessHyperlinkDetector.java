...
import org.otherone.robotframework.eclipse.editor.internal.rules.RFTVariableUtils;

/**
 * This hyperlink detector creates hyperlinks for variable accesses, e.g.
 * "  SomeKeyword ${variable}" --> "${variable}" is linked, but "Some ${arg} testcase" is not, and
 * "  [Arguments] ${foo}" isn't either.
 * 
 * @author xkr47
 */
public class RFTVariableAccessHyperlinkDetector implements IHyperlinkDetector {
  ...
  public IHyperlink[] detectHyperlinks(ITextViewer textViewer, IRegion region, boolean canShowMultipleHyperlinks) {
    ...
    // this is a hack - it doesn't detect properly if [Arguments] is INSIDE a variable or if [ is
    // quoted with \[
    ...
    if (start == -1) {
      if (line.startsWith("${") || line.startsWith(" ${")) {
        ...
        // testcase & keyword definitions fit into this category.. nothing interesting on those
        // lines
        return null;
      }
    }
    ...
    while (true) {
      ...
      if (offsetInLine < linkOffsetInLine) {
        // first variable is further down the line, so no more variables are reachable
        return null;
      }
